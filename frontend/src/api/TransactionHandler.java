 ...
 * Body POST contoh:
 * {
 *   "memberPhone": "08111111111",   ← opsional, kosong jika non-member
 *   "items": [
...
public class TransactionHandler implements HttpHandler {
    ...
    public TransactionHandler(TransactionService ts, ProductService ps, MemberService ms) {
    ...
    private void handleGet(HttpExchange ex, String id) throws IOException {
        if (id == null) {
            ...
            ApiServer.sendResponse(ex, 200, transactionsToJson(list));
        } else {
            Optional<Transaction> opt = transactionService.cariById(id);
            if (opt.isPresent()) {
                ApiServer.sendResponse(ex, 200, transactionToJson(opt.get()));
            } else {
                ApiServer.sendResponse(ex, 404, "{\"error\":\"Transaksi tidak ditemukan\"}");
            }
        }
    }
    ...
    private void handlePost(HttpExchange ex) throws IOException {
        ...
        try {
            ...
            String memberPhone = JsonParser.getStringOrNull(body, "memberPhone");
            Member member = null;
            if (memberPhone != null && !memberPhone.isBlank()) {
                member = memberService.cariByPhone(memberPhone).orElse(null);
            }

            // Buat transaksi baru
            Transaction transaksi = transactionService.buatTransaksi(member);

            // Parse array items: [{"productId":"PRD-0001","qty":2}, ...]
            String itemsJson = JsonParser.getArray(body, "items");
            String[] itemEntries = splitJsonArray(itemsJson);

            StringBuilder errors = new StringBuilder();
            for (String itemJson : itemEntries) {
                String productId = JsonParser.getString(itemJson, "productId");
                int qty          = JsonParser.getInt(itemJson, "qty");
                String err = transactionService.tambahItem(transaksi, productId, qty);
                if (err != null) errors.append(err).append("; ");
            }

            if (transaksi.getItems().isEmpty()) {
                ApiServer.sendResponse(ex, 400,
                        "{\"error\":\"Tidak ada item valid: " + ProductHandler.escapeJson(errors.toString()) + "\"}");
                return;
            }
            ...
            if (member != null) {
                memberService.catatTransaksi(member, transaksi.getTotalAfterDiscount());
            }

            ApiServer.sendResponse(ex, 201, transactionToJson(transaksi));
            ...
            items.append(String.format("""
                {
                  ...
                  "subtotal": %.0f
                    ...
                    ProductHandler.escapeJson(item.getProduct().getName()),
                    item.getQty(),
                    item.getPricePerItem(),
                    item.getSubtotal()
            ));
            if (i < itemList.size() - 1) items.append(",");
        }
        items.append("]");

        String memberJson = t.getMember() == null ? "null" : MemberHandler.memberToJson(t.getMember());
        ...
        return String.format("""
            {
              ...
              "total": %.0f
                ...
                t.getTransactionId(),
                ProductHandler.escapeJson(t.getFormattedDate()),
                memberJson,
                items.toString(),
                t.getSubtotal(),
                t.getMemberDiscountAmount(),
                t.getTotalAfterDiscount()
        );
    }

    /**
     * Split JSON array string menjadi array of JSON object strings.
     * Sederhana: split berdasarkan },{
     */
    private String[] splitJsonArray(String arrayJson) {
        if (arrayJson == null || arrayJson.isBlank() || arrayJson.equals("[]")) return new String[0];
